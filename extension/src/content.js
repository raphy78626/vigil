...
(() => {
  ...
  /* ------------------------------------------------------------------ */
  /* Visible text (full innerText including descendants)                */
  /* ------------------------------------------------------------------ */
  ...
  function getNthOfType(el) {
    ...
    const siblings = Array.from(el.parentElement.children).filter(c => c.tagName === tag);
  ...
  function getSiblingContext(el) {
    ...
    if (prev) {
      ...
      if (t && t.length <= 80) ctx.prevText = t;
    }
    if (next) {
      const t = (next.innerText || '').trim();
      if (t && t.length <= 80) ctx.nextText = t;
    }
